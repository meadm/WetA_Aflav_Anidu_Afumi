...
#variable is the file name but without "fumigatus"	
	NIDULANS=$(echo $FILE | sed 's/fumigatus//')
...
#cut the line so that only the ID is a variable		
		...
		if [ "$ID" == "name" ]; then
			...
			awk -F\t -v OFS='\t' -v ID="$ID" '$1 == ID {print $1,$2}' ../../nidulans\ RNAseq/BinGO/AspGD/nidulans$NIDULANS >> ../../GO_comparisons/fumigatus_nidulans$NIDULANS
		else
			:
		fi
	done < $FILE
